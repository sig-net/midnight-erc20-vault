...
describe("compactSerializedSize matches the compiler", () => {
  ...
  it("zero-size shapes really are zero bytes", () => {
    ...
    expect(compactSerializedSize({ kind: "vector", length: 0, element: { kind: "field" } })).toBe(
      0,
    );
  });
...
describe("twin roundtrip: compactDeserialize inverts compactSerialize", () => {
  it.each(SHAPES)("%s", (_name, type, value) => {
...
describe("twin rejections", () => {
  ...
  it("bounded uint and enum ranges on encode", () => {
    ...
    expect(() => compactSerialize({ kind: "uint", bound: 1 }, 1n)).toThrow(/exceeds Uint<0\.\.1>/);
    expect(() => compactSerialize({ kind: "enum", variants: 3 }, 3)).toThrow(/outside 0\.\.2/);
    expect(() => compactSerialize({ kind: "enum", variants: 3 }, -1)).toThrow(/outside 0\.\.2/);
  ...
  it("structural mismatches", () => {
    ...
    expect(() => compactSerialize(BUFFERS, { ...buffersValue, word: new Uint8Array(31) })).toThrow(
      /exactly 32 bytes/,
    );
    expect(() => compactSerialize(VECTORS_PLAIN, { ...vectorsPlainValue, nums: [1n] })).toThrow(
      /exactly 3 elements/,
    );
    expect(() => compactSerialize(TUPLE, [true, 0x1234n] as never)).toThrow(/exactly 3 elements/);
  });
  ...
  it("unknown struct fields are rejected on encode, mirroring descriptor strictness", () => {
    ...
    expect(() => compactSerialize(INNER, { ...innerValue, oK: false } as never)).toThrow(
      /unknown field 'oK'/,
    );
  });
});
...
describe("hostile field names (legal Compact identifiers, hostile to JS)", () => {
  ...
  it("decode materialises them as own properties, no prototype pollution", () => {
    ...
    expect(Object.getOwnPropertyDescriptor(decoded, "toString")?.value).toBe(6n);
  ...
  it("inherited properties do not satisfy a field: own properties only", () => {
    ...
    const patched = Object.fromEntries(Object.entries(noToString).map(([k, v]) => [k, BigInt(v)]));
    expect(() => compactSerialize(PROTO, patched as never)).toThrow(/missing field 'toString'/);
  });
});
...
describe("strict runtime descriptor validation (TypeScript is not enough)", () => {
  ...
  it("rejects unknown kinds at every entry point, never returns undefined", () => {
    ...
    expect(() => compactSerialize(banana, true)).toThrow(/unknown descriptor kind/);
    expect(() => compactDeserialize(banana, new Uint8Array(1))).toThrow(/unknown descriptor kind/);
  });
  ...
  it("rejects uint keys inherited through the prototype chain", () => {
    ...
    const inherited: object = Object.assign(Object.create({ bits: 8 }) as object, {
      kind: "uint",
    });
  ...
  it("uint form is exactly one of bits and bound", () => {
    ...
    expect(() => compactSerializedSize(bad({ kind: "uint", bound: (1n << 248n) + 1n }))).toThrow(
      /1\.\.2\^248/,
    );
    ...
    expect(() => compactSerializedSize(bad({ kind: "tuple", elements: { length: 1 } }))).toThrow(
      /must be an array/,
    );
  });
  ...
  it("rejects a non-Uint8Array buffer and a fractional padTo", () => {
    expect(() => compactDeserialize(PAIR, bad([1, 2, 3]) as never)).toThrow(/must be a Uint8Array/);
    expect(() => compactSerialize(PAIR, { a: 1n, b: 2n }, 24.5)).toThrow(/non-negative integer/);
  });

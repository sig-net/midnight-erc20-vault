...
import { fundingShare, perChildAmount, type RoleWallet } from "../src/setup/wallets.ts";

const DEPLOYER: RoleWallet = { label: "deployer", envVar: "DEPLOYER_SEED", shares: 3n };
const INVOKER: RoleWallet = { label: "invoker", envVar: "INVOKER_SEED", shares: 1n };
...
describe("fundingShare", () => {
  ...
  const CASES: readonly ShareCase[] = [
    {
      name: "every role unfunded: five shares for the children plus root's one",
      rootNight: 600n,
      unfunded: [DEPLOYER, INVOKER, MPC_RESPONDER],
      share: 100n,
    },
    {
...
describe("perChildAmount", () => {
  ...
  const CASES: readonly AmountCase[] = [
    ...
    { name: "the invoker receives one share", env: {}, share: 100n, child: INVOKER, amount: 100n },
    {
  ...
  it("refuses a FUND_CHILD_NIGHT that is not a non-negative integer", () => {
    expect(() => perChildAmount({ FUND_CHILD_NIGHT: "-1" }, 100n, INVOKER)).toThrow(
